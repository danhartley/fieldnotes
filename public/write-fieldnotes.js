...
import { previewTemplates, writeTemplates } from './common/templates.js'
...
const init = () => {
  ...
  const fnAutocompleteTitleInputText = d.getElementById(
    'fn-autocomplete-title-input-text'
  )
  const fnAutocompleteTitleDatalist = d.getElementById(
    'fn-autocomplete-title-data-list'
  )
  const importFieldNotesNotificationText = d.getElementById(
    'fetching-fieldnotes-notification-text'
  )
  ...
  const selectSectionTypeSection = d.getElementById(
    'select-section-type-section'
  )
  const selectionTypeBtns = selectSectionTypeSection.querySelectorAll('button')

  const updateFieldnotesStatusText = d.getElementById(
    'update-fieldnotes-status-text'
  )
  const updateFieldnotesCurrentStatusText = d.getElementById(
    'update-fieldnotes-current-status-text'
  )
  ...
  const iNatAutocompleteInputText = d.getElementById(
    'inat-autocomplete-input-text'
  )
  const iNatAutocompleteDatalist = d.getElementById(
    'inat-autocomplete-data-list'
  )

  const singleObservationsInputDate = d.getElementById(
    'single-observations-input-date'
  )
  const searchInatObservationsNotificationText = d.getElementById(
    'search-inat-observations-notification-text'
  )
  ...
  const rememberInatUserCheckbox = d.getElementById(
    'remember-inat-user-checkbox'
  )

  const exportFieldnotesNotificationText = d.getElementById(
    'export-fieldnotes-notification-text'
  )
  const exportFieldnotesInputCheck = d.getElementById(
    'export-fieldnotes-input-check'
  )

  draggableSections.addEventListener('dragover', dragoverHandler)
  draggableSections.addEventListener('drop', (e) =>
    dropHandler({
      e,
      globalWrite,
      draggableSections,
  ...
  const toggleView = ({ btn }) => {
    ...
    Array.from(d.getElementsByClassName('view-btn')).forEach((btn) => {
      btn.innerText = btn.innerText.toLowerCase() === 'show' ? 'hide' : 'show'
    })

    Array.from(d.getElementsByClassName('fieldnotes-view')).forEach((view) => {
  ...
  const searchInatObservations = async () => {
    try {
      ...
      globalWrite.fieldnotes.user =
        globalWrite.inatAutocompleteOptions.find((o) => o.id === 'users')
          ?.user || defaultUser
      ...
      setTimeout(() => {
        searchInatObservationsNotificationText.classList.toggle('hidden')
        searchInatObservationsNotificationText.innerText =
      ...
      globalWrite.fieldnotesStubs = globalWrite.fieldnotesStubsCollection.find(
        (stub) => stub.title === globalWrite.fieldnotes.title
      )
      ...
      if (rememberInatUserCheckbox.checked) {
        const {
          ...
          observations_count,
          species_count,
        } = globalWrite.observations[0].user
      ...
      d.querySelectorAll('.has-fieldnotes').forEach((section) =>
  ...
  const createSection = ({
    ...
    try {
      ...
      switch (writeTemplateId) {
        ...
        case 'xenocanto-write-template':
          ...
          input.addEventListener(
            'input',
            (e) => handleInputChangeEvent(e, addOrUpdateSectionBtn),
            true
          )
        ...
        case 'species-write-template':
          ...
          observer.observe(
            draggableSection.querySelector('.content-container'),
            {
              subtree: true,
              childList: true,
            }
      ...
      Array.from(draggableSections.getElementsByTagName('section')).forEach(
        (section) => {
      ...
      draggableSections.scrollIntoView({
        behavior: 'smooth',
        block: 'end',
        inline: 'end',
      })
      ...
      cancelActionBtn.addClickHandler({
        clickHandler: () => {
          ...
          Array.from(contentContainer.querySelectorAll('.edit')).forEach((el) =>
            el.classList.remove('hidden')
          )
          Array.from(
            contentContainer.querySelectorAll('.add:not(.edit)')
      ...
      switch (writeTemplateId) {
        ...
        case 'images-write-template': {
          ...
          ;[url1, title1, url2, title2, url3, title3].forEach(
            (input, index) => {
              input.addEventListener(
                'input',
                (e) =>
        ...
        case 'inat-lookup-write-template':
          ...
          globalWrite.inatAutocomplete =
            globalWrite.inatAutocompleteOptions.find(
              (option) => option.id === 'taxa'
            )
      ...
      (e) => {
        ...
        const parentTemplateId =
          globalWrite.fieldnotes.sections.find(
            (section) => section.writeTemplateId === writeTemplateId
          )?.writeParentTemplateId ||
          writeTemplates.find(
            (template) => template.templateId === writeTemplateId
          ).writeParentTemplateId
  ...
  const addOrUpdateSection = async ({
    ...
    const writeTemplate = writeTemplates.find(
      (template) => template.templateId === writeTemplateId
    )
    const previewTemplate = previewTemplates.find(
      (template) => template.templateId === writeTemplate.previewTemplateId
    )
    ...
    switch (writeTemplateId) {
      ...
      case 'species-write-template':
      case 'inat-lookup-write-template':
        sectionAddedOrUpdated = globalWrite.fieldnotes.sections.find(
          (t) => t.sectionIndex === sectionIndex
        )
        if (sectionToUpdate)
          sectionToUpdate.species = getOriginalTypeValues({
            globalWrite,
      ...
      case 'terms-write-template':
        ...
        if (sectionToUpdate)
          sectionToUpdate.terms = getOriginalTypeValues({
            globalWrite,
            section: sectionToUpdate,
            type: sectionToUpdate.type,
          })
      ...
      case 'images-write-template':
        ...
        if (sectionToUpdate)
          sectionToUpdate.images = getOriginalTypeValues({
            globalWrite,
            section: sectionToUpdate,
            type: sectionToUpdate.type,
          })
    ...
    // Show the preview section and hide the edit section
    Array.from(parent.querySelectorAll('.edit')).forEach((el) =>
      el.classList.remove('hidden')
    )
    Array.from(parent.querySelectorAll('.add:not(.edit)')).forEach((el) =>
  ...
  const enableSearchBtn = () => {
    ...
    const hasDate =
      singleObservationsInputDate.value.length > 0 &&
      ...
      Object.assign(notes, {
        ...
        user: {
          ...
          identifications_count:
            globalWrite.fieldnotes.user.identifications_count || 0,
        ...
        sectionOrder: globalWrite.fieldnotes.sections.map(
  ...
  const fetchFieldnotes = async () => {
    try {
      ...
      Object.assign(globalWrite, {
        fieldnotes: {
          ...
          sections: fieldnotes.sectionOrder.map((sectionIndex) => {
            return fieldnotes.sections.find(
              (section) => section.sectionIndex === sectionIndex
            )
          }),
        },
      ...
      globalWrite.nextSectionIndex =
        globalWrite.fieldnotes.sections.length > 0
          ? globalWrite.fieldnotes.sections
              .map((s) => s.sectionIndex)
              .sort(function (a, b) {
                return a - b
              })[globalWrite.fieldnotes.sections.length - 1] + 1
          : 0
      ...
      globalWrite.fieldnotes.sections.forEach((section) => {
        ...
        if (draggableSection.querySelector('.add:not(.edit)'))
          draggableSection
            .querySelector('.add:not(.edit)')
            .classList.add('hidden')

        const previewTemplate = previewTemplates.find(
          (template) => template.templateId === section.templateId
        )
        ...
        switch (section.templateId) {
          ...
          case 'observations-preview-template':
            ...
            speciesCheckboxes.forEach((checkbox) => {
              let hasMatch
              if (section.templateId === 'species-preview-template')
                hasMatch = section.species
                  .map((sp) => sp.taxon.name)
              ...
              if (section.templateId === 'observations-preview-template') {
                ...
                hasMatch =
                  section.templateId === 'observations-preview-template' &&
                  section.species
                    .map((sp) => sp.observation.id)
                    .includes(checkboxId)
              }
              if (hasMatch) {
            ...
            draggableSection.querySelector(
          ...
          case 'images-preview-template':
            section.images.forEach((img, i) => {
              if (img.src.length > 0) {
                d.querySelector(
                  `#section-id-${section.sectionIndex} #image-url-input-${i}`
                ).value = img.src
                d.querySelector(
                  `#section-id-${section.sectionIndex} #image-title-input-${i}`
                ).value = img.alt
              }
            })
            break
  ...
  const firebaseSignUpCheckbox = new CheckBoxComponent({
    ...
    clickHandler: (e) => {
      ...
      if (checkbox.checked) {
        ...
        signUpBtn.show()
        authenticateStateText.innerText =
          'You can sign up with a valid email and password.'
      } else {
  ...
  new ButtonComponent({
    ...
    clickHandler: async () => {
      ...
      if (response.success) {
        globalWrite.fieldnotesStubsCollection =
          globalWrite.fieldnotesStubsCollection.filter(
            (stub) => stub.id !== globalWrite.fieldnotesStubs.title
          )
