...
const init = async () => {
  const initGlobalRead = () => {
    ...
    Object.assign(globalRead, {
      ...
      templates: g.templates.filter((template) =>
        template.types.includes('inatSearch')
      ),
  ...
  // Filters and search
  const inatAutocompleteGroupContainer = d.getElementById(
    'inat-autocomplete-group-container'
  )
  ...
  const inatUseObservationOrSpeciesContainer = d.getElementById(
    'inat-use-observations-or-species-container'
  )
  ...
  let rbTestForGroup,
    rbInatAutocompleteGroup,
    rbInatUseObservationSpeciesCountGroup
  ...
  const createTaxaCheckboxGroup = () => {
    ...
    globalRead.ICONIC_TAXA.forEach((taxon) => {
      ...
      div.style.setProperty(
        'background-color',
        getTaxonGroupColour({ taxon: taxon.name.toLowerCase() })
      )
  ...
  const renderDisplayTemplate = ({ species }) => {
    try {
      ...
      if (species.length === 0) {
        showNotificationsDialog({
          message:
            'Your search returned no results. Try broadening your filters.',
      ...
      // Scroll to results
      Array.from(sectionsWithHeader)[0].scrollIntoView({
        behavior: 'smooth',
        block: 'start',
        inline: 'nearest',
      })
  ...
  const fetchInatSpecies = async () => {
    const filters = Array.from(
      d.getElementById('iconic-taxa-container').querySelectorAll('input')
    )

    globalRead.iconicTaxa = globalRead.ICONIC_TAXA.filter((taxon) =>
      filters
        .filter((t) => t.checked)
    ...
    const authenticatedUser = globalRead.inatAutocompleteOptions.find(
      (o) => o.id === 'users'
    )
    const user = authenticatedUser.isActive
      ? authenticatedUser
      : globalRead.user
    const place = globalRead.inatAutocompleteOptions.find(
      (o) => o.id === 'places'
    )
  ...
  rbInatAutocompleteGroup = createRadioBtnGroup({
    collection: globalRead.inatAutocompleteOptions.filter(
  ...
  rbInatUseObservationSpeciesCountGroup.forEach((rb) => {
    rb.addEventListener(
      'change',
      () =>
        (globalRead.useObservationsSpeciesCount =
          globalRead.useObservationsSpeciesCountOptions.find(
            (o) => o.id === rb.value
          ))
    )
  })
  ...
  d.addEventListener('DOMContentLoaded', (event) => {
    rbInatAutocompleteGroup.forEach((rb) => {
      rb.addEventListener('change', (e) => {
        ...
        iNatAutocompleteInputText.value = ''
        iNatAutocompleteInputText.setAttribute(
          'placeholder',
          globalRead.inatAutocomplete.placeholder
        )
      })
    })
