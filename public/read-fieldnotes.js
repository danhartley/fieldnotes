...
const init = async () => {
  const initGlobalRead = () => {
    ...
    Object.assign(globalRead, {
      ...
      template: g.templates.find(
        (template) => template.templateId === 'species-template'
      ),
    })
    return globalRead
  }
  ...
  const rememberLanguageCheckbox = d.getElementById(
  ...
  const createRadioBtnTemplateGroup = () => {
    const addHandlers = () => {
      const rbGroupTemplate = d.querySelectorAll(
        'input[name="display-option-template"]'
      )

      rbGroupTemplate.forEach((template) => {
        template.addEventListener('change', (e) => {
          globalRead.template = globalRead.templates.find(
          ...
          const toggleTestableTemplate = () => {
            globalRead.template = globalRead.templates.find(
              (t) => t.id === globalRead.template.pairedTemplateId
            )
            ...
            if (globalRead.template.targets) {
              ...
              rbTestForGroup.forEach((test) => {
                test.addEventListener('change', (e) => {
                  ...
                  globalRead.target = globalRead.template.targets.find(
                    (t) => t.id === testId
                  )
    ...
    globalRead.templates
      .filter((t) => !t.isTest)
      .forEach((t) => {
        if (
          t.requiresSpecies &&
          (!globalRead.species || globalRead.species.length === 0)
        )
          return
        ...
        if (
          globalRead.template &&
          globalRead.template.templateId === t.templateId
        ) {
          input.setAttribute('checked', true)
        }
      ...
      switch (globalRead.template.templateId) {
        ...
        case 'species-test-template':
          globalRead.species.forEach((sp, i) => {
            ...
            img.src = sp.taxon.default_photo.square_url.replace(
              'square',
              'small'
            )
        ...
        case 'fieldnotes-template': // Display metadata from the field trip to the fieldnotes
        {
          ...
          required[1].innerText = new Date(
            globalRead.fieldnotes.d1
          ...
          a.setAttribute(
            'href',
            `https://www.google.com/maps/place/${globalRead.fieldnotes.location.location}`
          )
          // Optional metadata
                  ...
                  try {
                    const clone = cloneSpeciesCardFromTemplate({
                      ...
                      species: checkForLocalisedCommonSpeciesNames({
                        globalSpecies: globalRead.species.find(
                          (s) => s.taxon.id === sp.taxon.id
                        ),
                        sectionSpecies: sp,
                      }),
                      index: i,
                ...
                section.terms.forEach(async (term) => {
                  try {
                    ...
                    if (term?.dd) {
                      ...
                      def = globalRead.terms.find(
                        (t) => t.dt === term.dt || t.dt === term
                      )
                    }
                    ...
                    if (def.dx) {
                      eg.textContent = 'e.g. '
                      dx.append(
                        Array.isArray(def.dx) ? def.dx.join(', ') : def.dx
                      )
                    } else {
                      dd.removeChild(div2)
                    }
  ...
  const updateScore = () => {
    ...
    if (scoreIncorrectTd)
      scoreIncorrectTd.innerText =
        globalRead.species.length - globalRead.template.score
  }
    ...
    try {
      ...
      const fieldnotes = {
        ...
        sections: response.data.sectionOrder.map((sectionIndex) => {
          return response.data.sections.find(
      ...
      const inatLookupSections =
        globalRead.fieldnotes.sections.filter(
          (s) => s.templateId === 'inat-lookup-preview-template'
        ) || []
      const inatLookupSpecies =
        inatLookupSections?.map((s) => s.species)?.flat() || []
      ...
      const inatLookupSpeciesByRank = inatLookupSpecies
        .filter((s) => s.taxon.name.indexOf(' ') > 0)
        .concat(
          inatLookupSpecies.filter((s) => s.taxon.name.indexOf(' ') === 0)
        )
      const inatLookupTaxaIds = inatLookupSpeciesByRank.map((s) => s.taxon.id)
      const inatLookupTaxaNames = inatLookupSpeciesByRank.map(
        (s) => s.taxon.name
      )

      const taxaIds = [
        ...new Set(
          globalRead.fieldnotes.taxa.map((t) => t.id).concat(inatLookupTaxaIds)
        ),
      ]
      const taxaNames = [
        ...new Set(
          globalRead.fieldnotes.taxa
            .map((t) => t.name)
      ...
      if (lessonFieldsetLegend) {
        lessonFieldsetLegend.scrollIntoView({
          behavior: 'smooth',
          block: 'start',
          inline: 'nearest',
        })
  ...
  globalRead.templates = g.templates.filter((template) =>
    template.types.includes('fieldnotes')
  )
  globalRead.template = globalRead.templates.find(
    (template) => template.templateId === 'fieldnotes-template'
  )
  ...
  new ButtonComponent({
    ...
    clickHandler: () => {
      Array.from(d.querySelectorAll('.grid')).forEach((grid) =>
        grid.classList.add('page-breaks')
      )
      print()
    },
  })
  ...
  const titlesObserver = new MutationObserver(() => {
    ...
    if (auth?.currentUser?.email) {
      firebaseAuthDisplay.innerText =
        'You can view your private and all published fieldnotes.'
    }
  })
