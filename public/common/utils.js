...
export const saveJson = ({ obj, title = 'fieldnotes', textOnly = false }) => {
  ...
  const styleFieldTripPhotos =
    'height:165px;width:220px;object-fit:cover;margin-right:.25rem;margin-bottom:.25rem;'
  const styleSpeciesPhotos =
    'height:170px;width:170px;object-fit:cover;margin-right:.25rem;margin-bottom:.25rem;'
  ...
  const str = textOnly ? JSON.stringify(filterByText(obj)) : JSON.stringify(obj)
...
*/
export const validateSlug = ({ pathname = '', slugs = [] }) => {
  let validSlugs, slug
